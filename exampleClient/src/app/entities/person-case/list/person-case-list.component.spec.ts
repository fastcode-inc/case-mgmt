...
import {
  IPersonCase,
  PersonCaseService,
  PersonCaseDetailsComponent,
  PersonCaseListComponent,
  PersonCaseNewComponent,
} from '../';

describe('PersonCaseListComponent', () => {
  let fixture: ComponentFixture<PersonCaseListComponent>;
  let component: PersonCaseListComponent;
  let el: HTMLElement;

  let d = new Date();
  let t = DateUtils.formatDateStringToAMPM(d);
  let constData: IPersonCase[] = [
    {
      caseId: 1,
      personId: 1,
      casesDescriptiveField: 1,
      personDescriptiveField: 1,
    },
    {
      caseId: 2,
      personId: 2,
      casesDescriptiveField: 2,
      personDescriptiveField: 2,
    },
  ];
  let data: IPersonCase[] = [...constData];

  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [PersonCaseListComponent, ListComponent],
        imports: [TestingModule],
        providers: [PersonCaseService, ChangeDetectorRef],
        schemas: [NO_ERRORS_SCHEMA],
      }).compileComponents();
    }));

    beforeEach(() => {
      fixture = TestBed.createComponent(PersonCaseListComponent);
  ...
  describe('Integration tests', () => {
      ...
      TestBed.configureTestingModule({
        ...
        imports: [
          ...
          RouterTestingModule.withRoutes([
            { path: 'personcase', component: PersonCaseListComponent },
            { path: 'personcase/:id', component: PersonCaseDetailsComponent },
          ]),
        ],
        providers: [PersonCaseService, ChangeDetectorRef],
      }).compileComponents();
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/personcase/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
