...
public class UsersAppServiceTest {
    ...
    public void findUsersById_IdIsNotNullAndIdDoesNotExist_ReturnNull() {
        ...
        Mockito.when(_usersRepository.findById(anyString())).thenReturn(nullOptional);
    ...
    public void findUsersById_IdIsNotNullAndIdExists_ReturnUsers() {
        ...
        Mockito.when(_userspreferenceRepository.findById(ID.toString())).thenReturn(userspreferenceOptional);
        Mockito.when(_usersRepository.findById(anyString())).thenReturn(usersOptional);

        Assertions
            .assertThat(_appService.findById(ID.toString()))
    ...
    public void deleteUsers_UsersIsNotNullAndUsersExists_UsersRemoved() {
        ...
        Mockito.when(_reportDashboardRepository.findByUsersId(anyString())).thenReturn(dvrList);

        List<Dashboardversion> dvList = new ArrayList<>();
        Mockito.when(_dashboardversionRepository.findByUsersId(anyString())).thenReturn(dvList);

        List<Reportversion> rvList = new ArrayList<>();
        Mockito.when(_reportversionRepository.findByUsersId(anyString())).thenReturn(rvList);
        Userspreference userspreference = mock(Userspreference.class);
        Optional<Userspreference> userspreferenceOptional = Optional.of((Userspreference) userspreference);
        Mockito.when(_usersRepository.findById(anyString())).thenReturn(usersOptional);
        Mockito.when(_userspreferenceRepository.findById(anyString())).thenReturn(userspreferenceOptional);
    ...
    public void find_ListIsNotEmpty_ReturnList() throws Exception {
        ...
        Userspreference userspreference = new Userspreference();
        Optional<Userspreference> userspreferenceOptional = Optional.of((Userspreference) userspreference);
        Mockito.when(_userspreferenceRepository.findById(any(String.class))).thenReturn(userspreferenceOptional);
    ...
    public void searchKeyValuePair_PropertyExists_ReturnBooleanBuilder() {
        ...
        builder.and(users.displayName.eq("xyz"));
    ...
    public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
        ...
        BooleanBuilder builder = new BooleanBuilder();
        builder.or(users.displayName.eq("xyz"));
    ...
    public void ParsedashboardsJoinColumn_KeysStringIsNotEmptyAndKeyValuePairDoesNotExist_ReturnNull() {
        ...
        String keyString = "15";
        joinColumnMap.put("usersUsername", keyString);
    ...
    @Test
    public void ParsepersonsJoinColumn_KeysStringIsNotEmptyAndKeyValuePairDoesNotExist_ReturnNull() {
        Map<String, String> joinColumnMap = new HashMap<String, String>();
        String keyString = "15";
        joinColumnMap.put("username", keyString);
        Assertions.assertThat(_appService.parsePersonsJoinColumn(keyString)).isEqualTo(joinColumnMap);
    }
