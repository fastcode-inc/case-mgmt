...
public class UsersroleAppService implements IUsersroleAppService {
    ...
    public boolean checkIfRoleAlreadyAssigned(Users foundUsers, Role foundRole) {
        List<Usersrole> usersRole = _usersroleRepository.findByUsersUsername(foundUsers.getUsername());
    ...
    protected BooleanBuilder searchKeyValuePair(
        ...
        for (Map.Entry<String, SearchFields> details : map.entrySet()) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("usersUsername")) {
                if (details.getValue().getOperator().equals("contains")) {
                    builder.and(
                        usersrole.usersUsername.likeIgnoreCase("%" + details.getValue().getSearchValue() + "%")
                    );
                } else if (details.getValue().getOperator().equals("equals")) {
                    builder.and(usersrole.usersUsername.eq(details.getValue().getSearchValue()));
                } else if (details.getValue().getOperator().equals("notEqual")) {
                    builder.and(usersrole.usersUsername.ne(details.getValue().getSearchValue()));
                }
            }
    ...
    public UsersroleId parseUsersroleKey(String keysString) {
        ...
        usersroleId.setUsersUsername(keyMap.get("usersUsername"));
        return usersroleId;
    }
}
