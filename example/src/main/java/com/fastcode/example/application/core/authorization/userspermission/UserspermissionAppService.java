...
public class UserspermissionAppService implements IUserspermissionAppService {
    ...
    public CreateUserspermissionOutput create(CreateUserspermissionInput input) {
        ...
        if (input.getUsersUsername() != null) {
            foundUsers = _usersRepository.findById(input.getUsersUsername()).orElse(null);
        } else {
            return null;
        }
    ...
    public boolean checkIfPermissionAlreadyAssigned(Users foundUsers, Permission foundPermission) {
        List<Userspermission> usersPermission = _userspermissionRepository.findByUsersUsername(
            foundUsers.getUsername()
        );
        ...
        for (int i = 0; i < list.size(); i++) {
            if (
                !(
                    ...
                    list.get(i).replace("%20", "").trim().equals("usersUsername")
                )
            ) {
    ...
    protected BooleanBuilder searchKeyValuePair(
        ...
        for (Map.Entry<String, SearchFields> details : map.entrySet()) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("usersUsername")) {
                if (details.getValue().getOperator().equals("contains")) {
                    builder.and(
                        userspermission.usersUsername.likeIgnoreCase("%" + details.getValue().getSearchValue() + "%")
                    );
                } else if (details.getValue().getOperator().equals("equals")) {
                    builder.and(userspermission.usersUsername.eq(details.getValue().getSearchValue()));
                } else if (details.getValue().getOperator().equals("notEqual")) {
                    builder.and(userspermission.usersUsername.ne(details.getValue().getSearchValue()));
                }
            }
        ...
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
            if (joinCol != null && joinCol.getKey().equals("usersUsername")) {
    ...
    public UserspermissionId parseUserspermissionKey(String keysString) {
        ...
        userspermissionId.setUsersUsername(keyMap.get("usersUsername"));
